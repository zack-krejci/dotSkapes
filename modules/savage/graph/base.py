...
from ..graphics.shapes import Line, Rectangle, Text, Path
...
class UnifiedGraph (BaseGraph):
    ...
    def formatSettings (self, settings):
        ...
        addAttr (settings, 'xAxisMaxWidth', float, 100.0)
        addAttr (settings, 'yAxisMaxWidth', float, 100.0)
        addAttr (settings, 'y2AxisMaxWidth', float, 100.0)

        addAttr (settings, 'horizontalGrid', boolean, False)
        addAttr (settings, 'verticalGrid', boolean, False)

        addAttr (settings, 'yAxisMaxLines', int, 1)
        ...
        addAttr (settings, 'canvasBorderStyle', str, 'square');
...
"""
    ...
    def boundingBox (self):
        if not self.settings.canvasBorder:
            return
        if self.settings.canvasBorderStyle == 'square':
            bbox = Rectangle (x =  self.canvas.x,
                              y = self.canvas.y,
                              width= self.canvas.width, 
                              height = self.canvas.height)
        elif self.settings.canvasBorderStyle == 'L':
            bbox = Path ()
            bbox.move (self.canvas.x, self.canvas.y + self.canvas.height)
            bbox.line (self.canvas.x, self.canvas.y)
            bbox.line (self.canvas.x + self.canvas.width, self.canvas.y)
